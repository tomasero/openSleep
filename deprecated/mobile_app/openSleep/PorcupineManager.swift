...
private class AudioInputEngine_AudioQueue: AudioInputEngine {
    ...
    func start() throws {
        ...
        self.recordingURL = self.recordingsManager.audioDirectoryURLwithTimestamp()
      print("dream url", self.recordingURL)
        self.recordingsManager.addRecording(categoryName: "Experiment", path: recordingURL!.absoluteString, length: 60)
        ...
        self.audioFile = try AVAudioFile(forWriting: self.recordingURL!, settings: audioRecorderSettings)
...
//            self.audioFile!.write(from:bufferRef.pointee.mAudioData.assumingMemoryBound )
